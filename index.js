...
const menuCategories = {
  appetizers: {
    name: 'Appetizers',
    items: [
      { id: 1, name: 'Spring Rolls', price: 1200, description: 'Crispy vegetable spring rolls' },
      { id: 2, name: 'Chicken Wings', price: 1800, description: 'Spicy buffalo wings' },
      { id: 3, name: 'Nachos', price: 1500, description: 'Loaded cheese nachos' }
    ]
  },
  mains: {
    name: 'Main Courses',
    items: [
      { id: 4, name: 'Jollof Rice', price: 2500, description: 'Traditional Nigerian jollof rice' },
      { id: 5, name: 'Fried Rice', price: 2300, description: 'Chinese-style fried rice' },
      { id: 6, name: 'Grilled Chicken', price: 3200, description: 'Herb-marinated grilled chicken' },
      { id: 7, name: 'Fish & Chips', price: 2800, description: 'Beer-battered fish with fries' }
    ]
  },
  drinks: {
    name: 'Beverages',
    items: [
      { id: 8, name: 'Coca Cola', price: 500, description: 'Chilled soft drink' },
      { id: 9, name: 'Fresh Juice', price: 800, description: 'Freshly squeezed orange juice' },
...
const formatOrder = (order) => {
  ...
  order.forEach((item, index) => {
    ...
    orderText += `   ${formatCurrency(item.price)} each = ${formatCurrency(item.price * item.quantity)}\n\n`;
  });
...
app.get('/', (req, res) => {
  ...
  if (req.session.chatHistory.length === 0) {
    addBotMessage(req, "🍽️ Welcome to Tasty Bites Restaurant! How can I help you today?");
...
app.post('/chat', async (req, res) => {
  ...
  if (!userInput) {
    return res.json({ success: false, message: "Please enter a valid option." });
  }
...
const handleMainMenu = (req, input) => {
  switch (input) {
    ...
    case '97':
      return formatOrder(req.session.currentOrder) + "\n\n" + getMainMenuOptions();
    
    case '0':
      return handleCancelOrder(req);
    
    default:
      return "❌ Invalid option. Please select from the available options:\n\n" + getMainMenuOptions();
  }
};
...
const handleCategorySelection = (req, input) => {
  switch (input) {
    ...
    default:
      return "❌ Invalid option. Please select from the available categories:\n\n" + getCategoryMenu();
  }
};
...
const handleMenuSelection = (req, input) => {
  ...
  if (!selectedItem) {
    return "❌ Invalid item selection. Please choose from the available items:\n\n" + 
           getMenuItems(req.session.selectedCategory);
  }
  
  // Add to current order
  const existingItem = req.session.currentOrder.find(item => item.id === selectedItem.id);
  if (existingItem) {
  ...
  return `✅ Added ${selectedItem.name} to your cart!\n\n` +
...
const handleScheduling = (req, input) => {
  ...
  switch (input) {
    case '1':
      ...
      return "✅ Order scheduled for immediate preparation.\n\n" +
    ...
    case '2':
      ...
      return "⏰ Order scheduled for 1 hour from now.\n\n" +
             "💳 Please choose payment method:\n" +
             "1️⃣ Pay with Paystack\n" +
             "0️⃣ Back to main menu";
...
const handleOrderHistory = (req) => {
  ...
  req.session.orderHistory.forEach((order, index) => {
    history += `Order #${index + 1} - ${new Date(order.date).toLocaleDateString()}\n`;
...
app.get('/payment/callback', async (req, res) => {
  ...
  try {
    ...
    if (response.data.data.status === 'success') {
      ...
      addBotMessage(req, "✅ Payment successful! Your order has been placed and will be prepared soon.");
      addBotMessage(req, getMainMenuOptions());
    ...
    addBotMessage(req, "❌ Payment verification failed. Please contact support.");
    addBotMessage(req, getMainMenuOptions());
    res.redirect('/');
  }
});
